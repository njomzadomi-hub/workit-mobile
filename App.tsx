...
import SendOfferScreen from'./src/screens/SendOfferScreen';
...
import SavedCandidatesScreen from'./src/screens/SavedCandidatesScreen';
...
function routePush(data:any){if(!navRef.isReady()||!protectedRoutesReady){pendingPushData=data;return}const type=String(data?.type||'').toLowerCase();const postId=data?.post_id||data?.job_post_id;const conversationId=data?.conversation_id;if((type.includes('message')||type.includes('chat'))&&conversationId){navRef.navigate('Chat',{conversationId});return}if(type.includes('message')||type.includes('chat')){navRef.navigate('Inbox');return}if((type==='application'||type.includes('interview')||type.includes('offer')||type.includes('hired'))&&postId){navRef.navigate('JobDetail',{jobId:postId});return}if(data?.market_item_id){navRef.navigate('MarketDetail',{id:data.market_item_id});return}navRef.navigate('Notifications')}
export default function App(){const[session,setSession]=useState<any>(null);const[loading,setLoading]=useState(true);useEffect(()=>{supabase.auth.getSession().then(({data})=>{setSession(data.session);setLoading(false)});const{data:sub}=supabase.auth.onAuthStateChange((_e,s)=>setSession(s));const notificationSub=Notifications.addNotificationResponseReceivedListener(response=>routePush(response.notification.request.content.data||{}));void Notifications.getLastNotificationResponseAsync().then(response=>{if(response)routePush(response.notification.request.content.data||{})}).catch(()=>{});return()=>{sub.subscription.unsubscribe();notificationSub.remove()}},[]);useEffect(()=>{protectedRoutesReady=!!session;if(session&&navRef.isReady()&&pendingPushData){const data=pendingPushData;pendingPushData=null;setTimeout(()=>routePush(data),0)}},[session]);if(loading)return<View style={{flex:1,backgroundColor:C.bg}}/>;return<NavigationContainer ref={navRef} onReady={()=>{protectedRoutesReady=!!session;if(session&&pendingPushData){const data=pendingPushData;pendingPushData=null;routePush(data)}}} theme={{...DarkTheme,colors:{...DarkTheme.colors,background:C.bg,card:C.bg,border:C.line,text:C.text,primary:C.violet}}}><StatusBar style="light"/>{session?<SessionGate/>:<AuthStack.Navigator screenOptions={{headerShown:false}}><AuthStack.Screen name="Login" component={LoginScreen}/><AuthStack.Screen name="Register" component={RegisterScreen}/><AuthStack.Screen name="Legal" component={LegalScreen}/></AuthStack.Navigator>}</NavigationContainer>}

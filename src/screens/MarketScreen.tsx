...
export default function MarketScreen(){
  ...
  useEffect(()=>{
    Promise.all([getMarket().catch(()=>({items:[]})), getFeed().catch(()=>({items:[]}))])
      .then(([market,feed])=>{
        const commerce=(market.items||[]).map((x:any)=>({
          ...x,
          source:'market',
          thumbnail_url:x.media_url,
          author:x.seller,
          author_title:x.seller?.title,
        }));
        const actionPosts=(feed.items||[]).filter((x:any)=>['job','donate'].includes(x.type)).map((x:any)=>({...x,source:'feed'}));
        const seen=new Set<string>();
        const merged=[...commerce,...actionPosts].filter((x:any)=>{ const k=`${x.source}:${x.id}`; if(seen.has(k))return false; seen.add(k); return true; });
        setItems(merged);
      })
  ...
  return <ScrollView style={s.page} contentContainerStyle={s.content} showsVerticalScrollIndicator={false}>
    ...
    {loading?<View style={s.empty}><Text style={s.emptyTitle}>Opening the bazaar…</Text></View>:visible.length===0?<View style={s.empty}><Text style={s.emptyTitle}>The market is opening.</Text><Text style={s.emptyBody}>Publish a service, product, job, lesson or donation post and it will appear here.</Text></View>:visible.map((item:any,i:number)=>{
      const meta=postMeta[item.type]||postMeta.service;
      const price=item.price_amount!=null?`${item.currency||'EUR'} ${Number(item.price_amount).toFixed(2)}`:null;
      return <TouchableOpacity key={`${item.source||'item'}-${item.id||i}`} style={s.item} activeOpacity={.88}>
        ...
        <View style={s.itemBody}><View style={s.itemTop}><Text style={[s.type,{color:meta.accent}]}>{meta.label}</Text><Text style={s.arrow}>↗</Text></View><Text numberOfLines={2} style={s.title}>{item.title||'Work opportunity'}</Text><Text numberOfLines={1} style={s.author}>{item.author?.full_name||'WORKIT member'}{item.author?.verified?' · Verified':''}</Text><View style={s.bottomRow}>{price?<Text style={s.price}>{price}</Text>:<View/>}<View style={s.cta}><Text style={s.ctaTxt}>{meta.cta}</Text></View></View></View>
      </TouchableOpacity>
    })}
  </ScrollView>
}

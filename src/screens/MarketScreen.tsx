...
const marketTypes = ['service','product','job','teach','donate'];

export default function MarketScreen(){
  const [items,setItems]=useState<any[]>([]);
  const [tab,setTab]=useState('All');
  useEffect(()=>{getFeed().then(r=>setItems((r.items||[]).filter((x:any)=>marketTypes.includes(x.type)))).catch(console.warn)},[])
  const visible=tab==='All'?items:items.filter(x=>x.type===tab.toLowerCase());
  return <ScrollView style={s.page} contentContainerStyle={s.content} showsVerticalScrollIndicator={false}>
    ...
    <View style={s.feature}><View style={s.featureTop}><Text style={s.featureEyebrow}>MARKETPLACE WITHOUT THE MIDDLEMAN FEEL</Text><Text style={s.spark}>✦</Text></View><Text style={s.featureTitle}>See the person. See the work. Then transact.</Text><Text style={s.featureBody}>Trust starts with video — every listing is connected to a real professional identity.</Text></View>
    <Text style={s.section}>Fresh in the bazaar</Text>
    {visible.length===0?<View style={s.empty}><Text style={s.emptyTitle}>The market is opening.</Text><Text style={s.emptyBody}>Publish a service, product, job, lesson or donation post and it will appear here.</Text></View>:visible.map((item:any,i:number)=>{
      const meta=postMeta[item.type]||postMeta.service;
      return <TouchableOpacity key={item.id||i} style={s.item} activeOpacity={.88}>
        ...
        <View style={s.itemBody}><View style={s.itemTop}><Text style={[s.type,{color:meta.accent}]}>{meta.label}</Text><Text style={s.arrow}>↗</Text></View><Text numberOfLines={2} style={s.title}>{item.title||'Work opportunity'}</Text><Text numberOfLines={1} style={s.author}>{item.author?.full_name||'WORKIT member'} · {item.author?.title||'Professional'}</Text><View style={s.cta}><Text style={s.ctaTxt}>{meta.cta}</Text></View></View>
      </TouchableOpacity>
    })}
  </ScrollView>
}
...
const s=StyleSheet.create({
 ...
 section:{color:C.text,fontSize:17,fontWeight:'800',marginBottom:12},item:{minHeight:142,borderRadius:20,backgroundColor:C.panel,borderWidth:1,borderColor:C.line,overflow:'hidden',flexDirection:'row',marginBottom:10},thumb:{width:112,height:'100%'},thumbPh:{backgroundColor:C.panel2,alignItems:'center',justifyContent:'center'},thumbPhTxt:{color:C.blue2,fontSize:34,fontWeight:'900'},itemBody:{flex:1,padding:14},itemTop:{flexDirection:'row',justifyContent:'space-between'},type:{fontSize:9,fontWeight:'900',letterSpacing:.8},arrow:{color:C.faint,fontSize:16},title:{color:C.text,fontSize:15,fontWeight:'800',lineHeight:20,marginTop:5},author:{color:C.muted,fontSize:10,marginTop:5},cta:{alignSelf:'flex-start',marginTop:10,paddingHorizontal:12,paddingVertical:7,borderRadius:R.pill,backgroundColor:C.text},ctaTxt:{color:'#000',fontSize:10,fontWeight:'900'},empty:{padding:22,borderRadius:20,borderWidth:1,borderColor:C.line,backgroundColor:C.panel},emptyTitle:{color:C.text,fontSize:16,fontWeight:'800'},emptyBody:{color:C.muted,fontSize:12,lineHeight:19,marginTop:6}
});
